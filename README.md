...
**Redis Advantages**:
- ✅ Atomic operations (`HINCRBYFLOAT`)
- ✅ Sub-millisecond latency for recent data
- ✅ Simple key-value model for aggregates
- ❌ Limited persistence (though RDB/AOF available)
- ❌ Memory-bound (expensive for large datasets)

**MongoDB Advantages**:
- ✅ Persistent, durable storage
- ✅ Rich query capabilities (date ranges, aggregations)
- ✅ Horizontal scalability (sharding)
- ❌ Higher latency than Redis
- ❌ More complex setup (indices, validation)

**Hybrid Approach**:
- Use Redis as a **write buffer** and **hot cache**
- Use MongoDB as **source of truth** for historical data
- Automatically offload old data to reduce Redis memory

### Why Background Tasks Instead of Queue Workers?

**Advantages**:
- ✅ Simpler deployment (no Celery/RabbitMQ)
- ✅ Shared application state (Redis/MongoDB clients)
- ✅ Lower overhead (no inter-process communication)

**Trade-offs**:
- ❌ No task retries (must implement manually)
- ❌ No distributed workers (single container)
- ❌ Harder to scale horizontally

For this use case (single CSV import, predictable load), background tasks are sufficient.

---

## Future Enhancements

1. **Monitoring**: Add Prometheus metrics for queue depth, processing rate, API latency
2. **Data Validation**: Validate CSV data before pushing to queue
3. **Error Recovery**: Implement dead-letter queue for failed transactions
4. **Authentication**: Add API key or JWT authentication
5. **Rate Limiting**: Protect API endpoint from abuse
6. **Horizontal Scaling**: Separate Aggregator/Dumper into independent services
7. **Redis Persistence**: Enable AOF/RDB for crash recovery
8. **Testing**: Unit tests, integration tests, load tests
9. **Observability**: Distributed tracing (OpenTelemetry)
10. **Data Retention**: Automated cleanup of old MongoDB records
